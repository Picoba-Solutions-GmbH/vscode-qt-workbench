...
// QML_SINGLETON: the QML engine creates exactly one of these, the first time
// QML touches it, and owns it. In QML you just write "AppTheme.background" --
// there is no "AppTheme { }" declaration anywhere.
//
// Every Q_PROPERTY below shares ONE notify signal (themeChanged). That is legal
// and common in Qt: flipping lightMode changes all the colours at once, so one
// signal is enough to refresh every binding.
